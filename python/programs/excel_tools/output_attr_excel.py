...
class CreateAttrExcel():
    ...
    def write_attr_excel(self):
        ...
        self.write_parm_info_sheet(wb['参数信息表'], self.table_info.table_chn_nm, self.table_info.table_eng_nm, self.parm_no)
        self.write_attr_info_sheet(wb['实体属性'], self.table_info)
        self.write_txn_info_sheet(wb['交易界面'], self.parm_no, self.parm_nm)
        self.write_page_field_sheet(wb['页面栏位'], self.parm_no, self.parm_nm, self.table_info)
    
        output_excel_nm = '参数设计属性信息_' + self.date + '_' + self.parm_nm + '.xlsx'
